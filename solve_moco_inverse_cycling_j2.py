"""
solve_moco_inverse_cycling_j2.py

Python port of solveMocoInverseCycling_J2.m
"""
...
def solve_moco_inverse_cycling_j2(
    pxx: str,
    t_initial: float,
    t_final: float,
    ee_weight: float,
    ...
    """
    Run MocoInverse for one participant using scheme J2.

    Parameters:
      solveMocoInverseCycling_J2(PXX, tInitial, tFinal, eeWeight, forceWeight, pathName)
    """
    # --- Files ---
    ...
    # Update inverse with final model.
    ...
    # Add knee joint reaction goal on walker_knee_r, expressed in tibia_r.
    jr_goal = osim.MocoJointReactionGoal("cost")
    jr_goal.setJointPath("/jointset/walker_knee_r")
    jr_goal.setLoadsFrame("child")
    jr_goal.setExpressedInFramePath("/bodyset/tibia_r")
    # minimize force-y and force-z
...
if __name__ == "__main__":
    ...
    ap.add_argument("pxx", help="Participant ID (e.g., P03)")
    ap.add_argument("t_initial", type=float)
    ap.add_argument("t_final", type=float)
    ap.add_argument("ee_weight", type=float)
    ap.add_argument("force_weight", type=float)
    ap.add_argument("path_name", help="Suffix (e.g., _J2_P03_result)")
    ap.add_argument("--base_dir", default=None,
                    help="Folder containing participant files")
    args = ap.parse_args()

    solve_moco_inverse_cycling_j2(
        args.pxx, args.t_initial, args.t_final, args.ee_weight, args.force_weight, args.path_name, base_dir=args.base_dir
    )
